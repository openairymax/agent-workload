# AirymaxRT 用户态工程（agent-runtim）

> **定位**：Airymax 双工程结构中的**用户态工程**大管理仓。与之对应的
> **内核态工程**为 [agent-linux]（AirymaxOS，Linux 6.6 + sched_tac + eBPF
> + io_uring + Rust）。两者构建层面零互相引用，通过共享契约层（SC）与
> 语义同源层（SS）协作（IRON-9 四层共享模型）。

## 结构

```
agent-runtim/                       ← 用户态工程（大管理仓，v0.1.3 起）
...
| 工程 | 大管理仓 | 内容 | 构建 |
|------|----------|------|------|
| 用户态 | agent-runtim | agentrt + ecosystem + products + sdk | agentrt CMake 为运行时构建入口；ecosystem/products/sdk 服务化整合（安装期拷贝 + 运行时协议协作） |
